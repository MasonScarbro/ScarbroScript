...
namespace ScarbroScriptLSP.Analysis
{
    ...
    public class UserBuiltMethod
    {
        public string name;
        public int numOfParams;

        public UserBuiltMethod(string name, int numOfParams)
        {
            this.name = name;
    ...
    public class ScopedObject
    {
        ...
        public List<UserBuiltMethod> Methods { get; set; }
        public bool IsClass { get; set; }

        public ScopedObject(object objectType, List<UserBuiltMethod> methods = null)
        {
            ObjectType = objectType;
            Methods = methods;
            IsClass = methods != null;
            if (ObjectType is ScopedObject classCall)
            {
                Methods = classCall.Methods;
                IsClass = true;
            }
        }
    }
    ...
    public class ClassChecker
    {
        ...
        {
            Program.logger.Log($"Trying to get natives for type {line}");
        ...
        public static List<CompletionItem> GetUserClassCompletions(List<UserBuiltMethod> classMethods)
        {
            List<CompletionItem> methodCompletions = new List<CompletionItem>();
            foreach (UserBuiltMethod _method in classMethods)
            {
                methodCompletions.Add(
                    new CompletionItem
                    {
                        Label = _method.name,
                        Detail = $"{_method.name} function | {_method.numOfParams} Parameters",
                        Documentation = "{{blank}}"
