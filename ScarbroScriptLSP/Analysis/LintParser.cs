...
namespace ScarbroScriptLSP.Analysis
{
    public class LintParser
    {
        public class LintParseError : FormatException 
        {
            ...
            public LintParseError(string message, int line, string problemChild) : base(message)
            {
                this.line = line;
        ...
        public LintParser(List<Token> tokens)
        {
            this.tokens = tokens;
        }
        ...
        private LintParseError Error(Token token, String message)
        {
            //Console.WriteLine($"Parser Error at line {token.line}: {message}");
            //Error(token, message);
            
            // We need a Token with some GIRTH (length) so if its an empty then just do the last token
            if (token.lexeme.Equals("")) token = Previous();
